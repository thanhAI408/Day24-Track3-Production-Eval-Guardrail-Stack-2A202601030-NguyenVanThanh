...
def run_ragas_50q(answers: list[dict]) -> list[RagasResult]:
    ...
    results = []
    for i, (a, pq) in enumerate(zip(answers, per_q)):
        results.append(RagasResult(
            question_id=a.get("id", i+1),
            distribution=a.get("distribution", "unknown"),
            question=a.get("question", ""),
            answer=a.get("answer", ""),
            contexts=a.get("contexts", []),
            ground_truth=a.get("ground_truth", ""),
            faithfulness=pq.faithfulness if hasattr(pq, 'faithfulness') else 0.0,
            answer_relevancy=pq.answer_relevancy if hasattr(pq, 'answer_relevancy') else 0.0,
            context_precision=pq.context_precision if hasattr(pq, 'context_precision') else 0.0,
            context_recall=pq.context_recall if hasattr(pq, 'context_recall') else 0.0,
        ))
    return results
...
def bottom_10(results: list[RagasResult]) -> list[dict]:
    ...
    for i, r in enumerate(bottom):
        diag, fix = DIAGNOSTIC_TREE.get(r.worst_metric, ("Unknown issue", "Investigate further"))
        output.append({
            "rank": i + 1,
...
def cluster_analysis(results: list[RagasResult]) -> dict:
    ...
    dist_totals = {}
    for dist in ["factual", "multi_hop", "adversarial"]:
        dist_totals[dist] = sum(matrix[m][dist] for m in matrix)

    dominant_dist = max(dist_totals, key=dist_totals.get) if dist_totals else "factual"
    metric_totals = {m: sum(matrix[m].values()) for m in matrix}
    dominant_metric = max(metric_totals, key=metric_totals.get) if metric_totals else "faithfulness"

    insight = (f"Distribution '{dominant_dist}' có nhiều failure nhất với {dist_totals.get(dominant_dist, 0)} cases. "
               f"Metric '{dominant_metric}' là điểm yếu chủ đạo. "
               f"Gợi ý: {DIAGNOSTIC_TREE.get(dominant_metric, ('', ''))[1]}")
