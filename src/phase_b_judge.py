...
'''
    ...
    try:
        ...
        return {
            "winner": result.get("winner", "tie") if result.get("winner") in {"A", "B", "tie"} else "tie",
            "reasoning": result.get("reasoning", ""),
            "scores": {
                "A": max(0.0, min(1.0, float(result.get("scores", {}).get("A", 0.0)))),
                "B": max(0.0, min(1.0, float(result.get("scores", {}).get("B", 0.0)))),
            }
        }
...
def swap_and_average(question: str, answer_a: str, answer_b: str) -> JudgeResult:
    ...
    # Convert pass2 scores back to original space
...
def cohen_kappa(judge_labels: list[int], human_labels: list[int]) -> float:
    ...
    judge_pos = judge_labels.count(1) / n if n > 0 else 0
    judge_neg = judge_labels.count(0) / n if n > 0 else 0
    human_pos = human_labels.count(1) / n if n > 0 else 0
    human_neg = human_labels.count(0) / n if n > 0 else 0
...
def bias_report(judge_results: list[JudgeResult]) -> dict:
    ...
    if position_bias_rate > 0.3:
        interpretation = "Position bias cao — nên dùng swap-and-average để giảm thiên lệch vị trí."
    elif position_bias_rate > 0.1:
        interpretation = "Position bias trung bình — có thể chấp nhận được với swap-and-average."
    else:
        interpretation = "Position bias thấp — judge tương đối ổn định, ít bị ảnh hưởng bởi vị trí."
